...
def update_product(prod_id, new_prod_name = None, new_prod_price = None, new_prod_quan = None):
    ...
    try:
        with DatabaseConnection(Config.DB_NAME) as connection:
            ...
            if new_prod_name is not None:
                ...
                cursor.execute(DBConfig.UPDATE_PRODUCT_NAME,(new_prod_name,prod_id,))
            if new_prod_price is not None:
                    # new_prod_quan=float(input(Config.ENTER_NEW_PROD_QUAN))
                cursor.execute(DBConfig.UPDATE_PRODUCT_QUANTITY,(new_prod_quan,prod_id,))
            if new_prod_quan is not None:
                    # new_prod_price=float(input(Config.ENTER_NEW_PROD_PRICE))
                cursor.execute(DBConfig.UPDATE_PRODUCT_PRICE,(new_prod_price,prod_id,))
        return True
    except Exception:
        return False
...
def reduce_prod_quan(p_id,p_quant):
    with DatabaseConnection(Config.DB_NAME) as connection:
        cursor=connection.cursor()
        cursor.execute(DBConfig.REDUCE_PROD_QUAN,(p_quant,p_id,))
